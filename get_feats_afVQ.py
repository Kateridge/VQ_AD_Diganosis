...
from models.models import model_CLS_CNN, model_CLS_VQCNN_Single
...
for fold_idx, (train_idx, test_idx) in enumerate(kfold_splits.split(np.arange(len(ADNI_data)))):
    ...
    with torch.no_grad():
        for i, data in enumerate(test_loader):
            ...
            # quant feats
            codebook_mapping, _, _ = CODEBOOK(feats)  # indices -> [bz, 3, 4, 3]
            codebook_mapping = rearrange(codebook_mapping, 'b n c -> (b n) c')
            feats_all = torch.cat([feats_all, codebook_mapping], dim=0)
    break

torch.save(feats_all.cpu(), f'VQCNN_{opt.modality}_feats.pt')
