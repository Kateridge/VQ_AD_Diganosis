...
from models.models import model_CLS_CNN, model_CLS_Transformer, model_CLS_CNN_Single, model_CLS_VQCNN_Single, \
    model_CLS_VQCNN_Transformer, model_CLS_VQCNN, model_CLS_VQCNN_MULTIMODEL_Transformer, \
...
for fold_idx, (train_idx, test_idx) in enumerate(kfold_splits.split(np.arange(len(ADNI_data)))):
    ...
    train_dataloader, val_dataloader, test_dataloader = setup_dataflow(train_idx, test_idx)
    if opt.model == 'MULTIMODEL':
        solver = model_CLS_VQCNN_MULTIMODEL_Transformer(opt, fold_idx)
    if opt.model == 'MULTIMODEL_withoutVQ':
        ...
        solver = None
    # start test
    res_fold = solver.start_test(test_dataloader, 2)
    # logging
